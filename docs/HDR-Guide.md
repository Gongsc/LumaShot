...
“设置 > HDR 校准”提供输出亮度与高光压缩控制，并优先使用当前 HDR 屏幕的缩略图实时预览。调整仅作用于 SDR 截图预览、PNG 和剪贴板；JPEG XR 始终保存未经校准压缩的原始浮点 HDR 数据。默认值为 85% 输出亮度和 20% 高光压缩，可随时恢复默认。
