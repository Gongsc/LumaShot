...
$portable = Join-Path $artifacts 'HDRSnapshot-portable-x64'
New-Item -ItemType Directory -Force $portable | Out-Null
$releaseExe = Join-Path $repoRoot 'bin\Release\HDRSnapshot.exe'
...
Compress-Archive -Path (Join-Path $portable '*') -DestinationPath (Join-Path $artifacts 'HDRSnapshot-portable-x64.zip') -Force
...
if ($isccPath) {
    & $isccPath (Join-Path $repoRoot 'installer\HDRSnapshot.iss')
