...
bool IsStartupConfigured() noexcept {
    ...
    DWORD type{}, size{};
    const bool exists = RegQueryValueExW(key, L"HDRSnapshot", nullptr, &type, nullptr, &size) == ERROR_SUCCESS && type == REG_SZ && size > sizeof(wchar_t);
    RegCloseKey(key);
    return exists;
}
}
...
int App::run() {
    ...
    cls.hCursor = LoadCursorW(nullptr, IDC_ARROW); cls.hIcon = LoadIconW(instance_, MAKEINTRESOURCEW(IDI_HDRSNAPSHOT)); cls.hIconSm = cls.hIcon; RegisterClassExW(&cls);
    hwnd_ = CreateWindowExW(WS_EX_TOOLWINDOW, WindowClassName, L"HDRSnapshot", WS_OVERLAPPED, 0, 0, 0, 0, nullptr, nullptr, instance_, this);
    if (!hwnd_) return 1;
...
void App::addTrayIcon() {
    ...
    appIcon_ = static_cast<HICON>(LoadImageW(instance_, MAKEINTRESOURCEW(IDI_HDRSNAPSHOT), IMAGE_ICON,
                                             GetSystemMetrics(SM_CXSMICON), GetSystemMetrics(SM_CYSMICON), LR_DEFAULTCOLOR));
    if (!appIcon_) appIcon_ = CreateAppIcon(GetSystemMetrics(SM_CXSMICON));
    tray_.hIcon = appIcon_; wcscpy_s(tray_.szTip, L"HDRSnapshot");
...
void App::finishCapture(std::unique_ptr<CapturePayload> payload) {
    ...
    OverlayWindow overlay(instance_, std::move(*payload->frames), settings_.lastCaptureMode, language_, settings_.includeCursor);
...
bool App::exportCapture(HWND owner, OverlayAction action, const CaptureFrameSet& frames, RectI selection, const AnnotationDocument& annotations) {
    try {
        ...
        if (action == OverlayAction::Copy) {
            ImageBgra8 image = ColorPipeline::toneMapToSdr(source); AnnotationRenderer::renderSdr(image, annotations);
            if (!ClipboardService::copy(owner, image)) { MessageBoxW(owner, Localized(StringId::ClipboardFailed, language_).data(), L"HDRSnapshot", MB_OK | MB_ICONERROR); return false; }
        ...
        else { ImageBgra8 image = ColorPipeline::toneMapToSdr(source); AnnotationRenderer::renderSdr(image, annotations); ImageExporter::savePng(choice->path, image); }
        ...
        MessageBoxW(owner, Localized(message, language_).data(), L"HDRSnapshot", MB_OK | MB_ICONERROR); return false;
    }
}
...
void App::applyStartupSetting() noexcept {
    ...
    if (settings_.launchAtLogin) {
        ...
        RegSetValueExW(key, L"HDRSnapshot", 0, REG_SZ, reinterpret_cast<const BYTE*>(command.c_str()), static_cast<DWORD>((command.size() + 1) * sizeof(wchar_t)));
    } else RegDeleteValueW(key, L"HDRSnapshot");
    RegCloseKey(key);
}
