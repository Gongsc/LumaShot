...
bool TryDirect2DToneMap(const ImageF16& source, ImageBgra8& result) noexcept {
    try {
        ...
        if (FAILED(toneMap->SetValue(D2D1_HDRTONEMAP_PROP_INPUT_MAX_LUMINANCE, measuredMax)) ||
            FAILED(toneMap->SetValue(D2D1_HDRTONEMAP_PROP_OUTPUT_MAX_LUMINANCE, 80.0f)) ||
        ...
        if (FAILED(context->CreateEffect(CLSID_D2D1WhiteLevelAdjustment, &whiteLevel))) return false;
        whiteLevel->SetInputEffect(0, toneMap.Get());
        // A captured HDR desktop is already scaled to the display's SDR-white
        // level. Convert that display-referred white back to the 80-nit scRGB
        // reference before encoding an SDR file. The effect's scale is
        // InputWhiteLevel / OutputWhiteLevel, hence the apparently reversed
        // property order here.
        if (FAILED(whiteLevel->SetValue(D2D1_WHITELEVELADJUSTMENT_PROP_INPUT_WHITE_LEVEL, 80.0f)) ||
            FAILED(whiteLevel->SetValue(D2D1_WHITELEVELADJUSTMENT_PROP_OUTPUT_WHITE_LEVEL, std::max(80.0f, source.sdrWhiteLevelNits)))) return false;

        ComPtr<ID2D1Effect> colorManagement;
        if (FAILED(context->CreateEffect(CLSID_D2D1ColorManagement, &colorManagement))) return false;
        colorManagement->SetInputEffect(0, whiteLevel.Get());
...
ImageBgra8 ColorPipeline::toneMapToSdr(const ImageF16& source) {
    ImageBgra8 direct2d;
    if (source.width > 0 && source.height > 0 && TryDirect2DToneMap(source, direct2d)) return direct2d;
    ImageBgra8 result{source.width, source.height};
    result.pixels.resize(static_cast<std::size_t>(source.width) * source.height * 4);
    const float peak = std::max(1.0f, source.maxLuminanceNits / 80.0f);
    auto encode = [peak, hdr = source.hdr](float linear) {
