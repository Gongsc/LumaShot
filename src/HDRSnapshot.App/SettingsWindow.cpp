...
namespace {
constexpr wchar_t ClassName[] = L"HDRSnapshot.Settings";
...
SettingsWindow::SettingsWindow(HINSTANCE instance, HWND owner, AppSettings settings)
    : instance_(instance), owner_(owner), settings_(settings), displayLanguage_(ResolveLanguage(settings.language)) {}
...
LRESULT SettingsWindow::handleMessage(UINT message, WPARAM wparam, LPARAM lparam) {
    ...
    case WM_CREATE: {
        ...
        if (windowDpi != dpi_) {
            ...
            RECT desired{0, 0, scale(560), scale(420)};
    ...
    case WM_DRAWITEM:
        ...
        if (wparam == SaveButton || wparam == CancelButton) {
...
void SettingsWindow::createControls() {
    ...
    for (HWND control : {languageText, language, hotkeyText, hotkey, cursorBox, startupBox, saveButton, cancelButton}) {
...
void SettingsWindow::layoutControls() {
    ...
    MoveWindow(GetDlgItem(hwnd_, LanguageLabel), scale(44), scale(137), scale(180), scale(24), TRUE);
    MoveWindow(GetDlgItem(hwnd_, LanguageCombo), inputX, scale(128), inputWidth, scale(220), TRUE);
    MoveWindow(GetDlgItem(hwnd_, HotkeyLabel), scale(44), scale(184), scale(180), scale(24), TRUE);
    MoveWindow(GetDlgItem(hwnd_, HotkeyControl), inputX, scale(176), inputWidth, scale(34), TRUE);
    MoveWindow(GetDlgItem(hwnd_, CursorCheckbox), scale(44), scale(278), client.right - scale(88), scale(25), TRUE);
    MoveWindow(GetDlgItem(hwnd_, StartupCheckbox), scale(44), scale(309), client.right - scale(88), scale(25), TRUE);
...
void SettingsWindow::paint() {
    ...
    FillRoundedRect(dc, {scale(24), scale(88), client.right - scale(24), scale(224)},
                    scale(12), card, border);
    FillRoundedRect(dc, {scale(24), scale(238), client.right - scale(24), scale(344)},
                    scale(12), card, border);
    ...
    DrawTextW(dc, captureText.data(), static_cast<int>(captureText.size()), &captureCaption,
              DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_NOPREFIX);
    RECT behaviorCaption{scale(44), scale(250), client.right - scale(44), scale(273)};
