...
std::filesystem::path SettingsStore::DefaultPath() {
    ...
    CoTaskMemFree(raw);
    return result / L"HDRSnapshot" / L"settings.json";
}
...
AppSettings SettingsStore::load() const noexcept {
    ...
    try {
        ...
        if (const auto value = UIntValue(json, "schemaVersion")) result.schemaVersion = static_cast<int>(*value);
        if (result.schemaVersion != AppSettings::CurrentSchemaVersion) return AppSettings{};
